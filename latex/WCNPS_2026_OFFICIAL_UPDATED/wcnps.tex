...
5 & Comp & 1.010 & 1.010 & 0.000 & -14.2 & -14.1 & -0.1 \\
8 & Comp & 1.010 & 1.010 & 0.000 & -11.8 & -11.8 & -0.0 \\
12 & PQ & 1.057 & 1.057 & -0.000 & -14.9 & -14.9 & -0.0 \\
30 & PQ & 0.992 & 0.992 & -0.000 & -17.7 & -17.6 & -0.1 \\
\bottomrule
\end{tabular}}
\end{table}
...
\textbf{Py} & \textbf{ANA} & \textbf{Diff} \\
\midrule
1 & Slack & 261.0 & 261.0 & -0.0 & -15.3 & -16.5 & +1.2 \\
2 & PV & 40.0 & 40.0 & +0.0 & 51.6 & 49.6 & +2.0 \\
5 & Comp & 0.0 & 0.0 & +0.0 & 37.8 & 36.0 & +1.8 \\
8 & Comp & 0.0 & 0.0 & +0.0 & 38.8 & 37.3 & +1.5 \\
11 & Comp & 0.0 & 0.0 & +0.0 & 16.3 & 16.2 & +0.1 \\
13 & Comp & 0.0 & 0.0 & +0.0 & 10.8 & 10.6 & +0.2 \\
\bottomrule
\end{tabular}}
\end{table}
...
Starting from approximately 283.40 MW, the system reached an MLP of 838.44 MW at $\lambda\approx2.9585$. The corresponding increase relative to the base load is 195.85\%. The ANAREDE reference reports an MLP of approximately 836 MW \cite{madureira2023avaliacao}; therefore, the Python result is 2.44 MW higher, corresponding to a relative deviation of only 0.29\%. Considering the distinct solver implementations and convergence criteria, the results show very close agreement in the estimated loadability boundary.
...
\begin{figure}[!t]
    ...
    \caption{IEEE 30-bus PV curves obtained with the developed Python tool; the MLP is 838.44 MW.}
    \label{fig:curvapv}
\end{figure}
...
Table~\ref{tab:spearman} shows an almost perfect correlation between FVSI and $L_{mn}$ ($\rho_s=0.9943$). NLSI is also strongly correlated with FVSI and $L_{mn}$, with coefficients of 0.9118 and 0.9167, respectively. NVSI provides a completely distinct ordering, with correlations near zero: -0.0352 with FVSI, -0.0282 with $L_{mn}$, and 0.1155 with NLSI. These results indicate that FVSI and $L_{mn}$ provide largely redundant branch-ranking information in this case, while NVSI contributes a fundamentally different response.
...
\midrule
FVSI     & 1.0000 & 0.9943 & 0.9118 & -0.0352 \\
$L_{mn}$& 0.9943 & 1.0000 & 0.9167 & -0.0282 \\
NLSI     & 0.9118 & 0.9167 & 1.0000 & 0.1155 \\
NVSI     & -0.0352 & -0.0282 & 0.1155 & 1.0000 \\
\bottomrule
\end{tabular}
\end{table}
...
Contingency severity is ranked through the resulting $\lambda_{\max}$: a lower value indicates that the post-contingency system reaches its loading boundary sooner. Table~\ref{tab:n1_ranking} lists the ten most severe line outages. Line 0 is the most restrictive event, reducing $\lambda_{\max}$ to 1.2583. Lines 35 and 3 are the next most severe contingencies, with $\lambda_{\max}$ equal to 2.0350 and 2.2082, respectively.
...
This work presented an open-source Python framework for static voltage-stability assessment using a step-refined loadability search and five established VSIs. The IEEE 30-bus model was cross-compared with ANAREDE and reached an MLP of 838.44 MW, corresponding to a 195.85\% increase relative to the 283.40 MW base load and a minimal 0.29\% deviation from the 836 MW reference result. The voltage trajectory identifies Bus 30 as the weakest region near the loading boundary.
...
The N-1 study further demonstrated that the intact-network margin is insufficient to characterize the most restrictive condition. When line outages and generator reactive-power limits are considered, the available loadability can decrease substantially; among the analyzed outages, Line 0 produced the lowest $\lambda_{\max}$. These results support a combined methodology in which the loadability boundary provides the reference for system stress, VSIs provide element-level monitoring information, and contingency analysis identifies operating conditions that can reduce the available margin. The numerical values remain dependent on the adopted load-growth direction, generation redispatch, reactive-power limits, transformer settings, and power-flow convergence criteria.
