...
# ==============================================================================
# SCRIPT DE EXECUÇÃO AUTOMATIZADA - WCNPS 2026 (VERSÃO SPF - RETROCOMPATÍVEL)
# ==============================================================================

def main():
    print("="*80)
    print(" INICIANDO BATERIA DE EXPERIMENTOS DO ARTIGO WCNPS 2026 (SPF)")
    print("="*80)
    start_time = time.time()

    # Definir pasta base
    # O usuário optou por manter "outputs" para sobrescrever as tabelas/resultados
    base_results_dir = "outputs"
    os.makedirs(base_results_dir, exist_ok=True)

    # Configuracao Padrao SPF
    config_default = {
        ...
        'solver_max_iter': 20,
        'solver_tol': 0.1,
        'cpf_mode': 'spf',   # SPF Retrocompatível (Solicitado pelo usuário)
    }
    ...
    for i, exp in enumerate(experiments, 1):
        ...
        print("\n-> [2/2] Iniciando Análise N-1 (Com QLim Pandapower - Strict)...")
        net_n1 = exp['net_func']()
        if exp['adjust_gen']:
        ...
        # Conforme solicitado, usar o Qlim do pandapower ('strict')
        config_n1['qlim_mode'] = 'strict'
    ...
    print(f"\n{'='*80}")
    print(f"BATERIA DE EXPERIMENTOS WCNPS (SPF) CONCLUÍDA EM {mins}m {secs:.2f}s!")
    print(f"Todos os resultados salvos em: {base_results_dir}/")
    print(f"{'='*80}")
    
    print("\nIniciando formatação e geração dos dados para o artigo...")
    generate_article_data.main(base_dir=base_results_dir)
